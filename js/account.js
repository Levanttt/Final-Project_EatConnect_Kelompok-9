  ...
  document.querySelectorAll('.edit-button').forEach(button => {
    button.addEventListener('click', function() {
      ...
      if (label.includes('Telepon')) {
        const newPhone = prompt('Masukkan nomor telepon baru:', '0851-2987-2230');
        if (newPhone && newPhone.trim() !== '') {
          ...
          valueElement.querySelector('.edit-button').addEventListener('click', arguments.callee);
        }
...
function updateWeeklyProgress() {
  ...
  if (firstProgressCard) {
    ...
    if (valueElement) valueElement.textContent = `${checkedCount}/${totalCount} Hari`;
